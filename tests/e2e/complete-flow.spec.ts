...
test.describe("Complete User Flow", () => {
  ...
  test("complete user journey: register → login → profile update → logout", async ({
    page,
  }) => {
    // 1. 註冊新使用者
    ...
    // 6. 驗證資料已更新
    // 等待表單重新載入完成
    await page.waitForSelector('input[name="name"]:not([disabled])', {
      timeout: 10000,
    });
  ...
  test("complete internationalization flow", async ({ page }) => {
    ...
    await page.goto("/en");
    await expect(
      page.locator("text=Welcome to Next.js 15 Enterprise App"),
    ).toBeVisible({ timeout: 10000 });

    // 2. 切換到中文
    await commands.switchLanguage("zh");
    await expect(
      page.locator("text=歡迎使用 Next.js 15 企業級應用"),
    ).toBeVisible({ timeout: 10000 });
  ...
  test("complete responsive design flow", async ({ page }) => {
    ...
    const isMobileMenuVisible = await page
      .locator(".md\\:hidden nav.flex.flex-col")
      .isVisible()
      .catch(() => false);
    ...
    // 確認行動版選單初始狀態應該是關閉的
    await expect(
      page.locator(".md\\:hidden nav.flex.flex-col"),
    ).not.toBeVisible();
    ...
    await page.waitForSelector(
      '.md\\:hidden nav.flex.flex-col a[href*="/dashboard"]',
      { timeout: 10000 },
    );
    ...
    await page.waitForSelector(
      '.md\\:hidden nav.flex.flex-col a[href*="/profile"]',
      { timeout: 10000 },
    );
  ...
  test("complete error handling flow", async ({ page }) => {
    ...
    // 4. 重置網路條件並測試 404 錯誤
    await commands.resetNetworkConditions();
    await page.waitForTimeout(1000); // 等待網路重置
    ...
    try {
      ...
      // 如果 404 頁面導航失敗，直接導航到首頁測試錯誤恢復
      console.log(
        "404 page navigation failed, testing error recovery directly",
      );
    }
  ...
  test("complete accessibility flow", async ({ page, browserName }) => {
    test.skip(
      browserName === "webkit",
      "WebKit has different focus behavior for keyboard navigation",
    );
    ...
    // 2. 檢查 ARIA 標籤
    await expect(page.locator("[aria-label]")).toHaveCount(4, {
      timeout: 10000,
    }); // 檢查 ARIA 標籤數量
